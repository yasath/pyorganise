...
def error_log(string):
    tk_instance.configure(state="normal")
    tk_instance.insert("end", "\n")
    tk_instance.insert("end", string, "error")
    tk_instance.yview("end")
    tk_instance.configure(state="disabled")
...
def extra_id(file_path, file_category, verbose_option, tk_label):
    ...
    if extension == "mp3":
        ...
        if mp3_file.info.length > 30:
            ...
            matched = music_tag.acoustid_match(ACOUSTID_KEY, file_path[0] +
                                               file_path[1])
            if None in matched:
                error_log("The 'fpcalc' executable was not found, so songs " +
                          "cannot be recognised. " +
                          "Brief instructions are available online at: " +
                          "v.gd/fpcalc")
                ...
                try:
                    ...
                    error_log("ID3 tagging failed so metadata was not embedded")
            ...
            error_log("'{0}' is shorter than 30 seconds so will not be"
    ...
    if extension in ["jpg", "jpeg", "tiff", "tif"]:
        ...
        if not current_image.has_exif:
            error_log("'{0}' does not contain EXIF data so will not be"
    ...
    if extension in ["docx", "pptx", "pdf"]:
        ...
        if not search("[0-9][0-9][0-9][0-9].[0-9][0-9].[0-9][0-9]",
            ...
            error_log("'{0}' already contains a date so will not be renamed"
                      .format(original_filename))
    ...
    if extension == "vcf":
        ...
        if not new_filename:
            error_log("'{0}' does not contain FN: variable so contact name"
